...
# Usage: `ds_dup`
#
# Duplicate the top item of the stack on top of it.
#
ds_dup() { ds_push "${DS[-1]}"; }

# Usage: `ds_dup_n` `[n]`
#
# Just like `ds_dup`, but do it `n` times. `n` defaults to 1.
#
ds_dup_n() {
...
#
ds_swap() { local tmp=${DS[-2]}; DS[-2]=${DS[-1]}; DS[-1]=$tmp; }

# Usage: `ds_echo`
#
# Echo the top item on the stack.
#
ds_echo() { printf "%s\n" "${DS[-1]}"; }

# Usage: `ds_echo_pop`
#
# Pop and echo the top item on the stack.
#
